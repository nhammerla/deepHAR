...
--]]
function optim.cgcuda(opfunc, x, config, state)
   -- parameters
   ...
   local df1 = state.df1 or torch.CudaTensor()
   local df2 = state.df2 or torch.CudaTensor()
   local df3 = state.df3 or torch.CudaTensor()
   local tdf
   ...
   -- search direction
   local s = state.s or torch.CudaTensor()
   s:resizeAs(x)

   -- we need a temp storage for X
   local x0 = state.x0 or torch.CudaTensor()
   local f0 = 0
   local df0 = state.df0 or torch.CudaTensor()
   x0:resizeAs(x)
   df0:resizeAs(x)
